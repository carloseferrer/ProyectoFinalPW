...
if(isset($_POST['reportebtnProduccion'])){
    ...
    $pdf->SetTitle('Informe del la Produccion');
    ...
    $pdf->Cell(100,6,'LISTA DE PRODUCCION',0,0,'C');
    ...
    $pdf->Cell(30,6,'Precio Produccion',1,0,'C',1);
    $pdf->Cell(30,6,'Materiales',1,0,'C',1);
    $pdf->Cell(15,6,'Unidades',1,0,'C',1);
    $pdf->Cell(14,6,'Total',1,0,'C',1);
    $pdf->Cell(20,6,'Estado',1,0,'C',1);
    $pdf->Cell(28,6,'Fecha de Registro',1,1,'C',1); 
    ...
    $sqlOrdenCompra = ("SELECT * FROM produccion");
    //$sqlTrabajadores = ("SELECT * FROM trabajadores");
    $query = mysqli_query($con, $sqlOrdenCompra);
    
    while ($dataRow = mysqli_fetch_array($query)) {
            $pdf->SetX(23);
            $total = $dataRow['unidades'] * $dataRow['mano_obra'];
            $pdf->Cell(25,6,$dataRow['nombre_producto'],1,0,'C');
            $pdf->Cell(30,6,$dataRow['mano_obra'],1,0,'C');
            $pdf->Cell(30,6,$dataRow['materiales'],1,0,'C');
            $pdf->Cell(15,6,$dataRow['unidades'],1,0,'C');;
            $pdf->Cell(14,6,$total.'$',1,0,'C');
            $pdf->Cell(20,6,$dataRow['estado'],1,0,'C');
            $pdf->Cell(28,6,(date('m-d-Y', strtotime($dataRow['fecha_registro']))),1,1,'C');
        }
    ...
    $pdf->Output('Reporte_Produccion_'.date('d_m_y').'.pdf', 'I'); 
